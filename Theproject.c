...
#include<malloc.h>
#include<string.h>
float Grandtsp,Grandtcp,Grandprofit;
struct node 
{
	int itemcode,Qsold,icr,limit;
	char itemname[50];
...
{
	int i=1,code,quan,num,restlim,restincr;
	...
	while(i<=num)
	{
		...
		scanf("%d",&quan);
		nn->No_item=quan;
		printf("Enter restock limit and number of units to restock by:");
		scanf("%d%d",&restlim,&restincr);
		nn->limit=restlim;
		nn->icr=restincr;
		printf("Enter Unit cost price:\n");
		scanf("%f",&cp);
		nn->CostPrice=cp;
		printf("Enter Unit sale price:\n");
		scanf("%f",&sp);
		nn->SellingPrice=sp;
...
{
 	struct node *p;	
	Grandtsp=0,Grandtcp=0,Grandprofit=0;
	p=start;
	while(p!=NULL)
	{
...
struct node *restock(struct node *start)
{
	struct node *p;
	p=start;
	while(p!=NULL)
	{
		if((p->No_item)==(p->limit)||(p->No_item)<(p->limit))
			{
				printf("The quantity of %s is less and needs to be restock\nCurrent Stock of %s is %d:\nSelf restock activated\n",p->itemname,p->itemname,p->No_item);	
				p->No_item=p->No_item+p->icr;
				p->TCP=p->TCP+(p->CostPrice)*(p->No_item);
			}
			p=p->next;
	}
	return start;
...
struct node *display(struct node *start)
{
	...
	while(p!=NULL)
	{
		printf("%s\t\t%d\t\t%d\t\t%.3f\t\t%.3f\t\t%d\t\t%.3\t\t%.3\t\t%.3\n",p->itemname,p->itemcode,p->No_item,p->CostPrice,p->SellingPrice,p->Qsold,p->TCP,p->TSP,p->profit);
		p=p->next;	
	}
	return start;
}
	...
	do{
		...
		switch(choice)
		{
			...
			case 5:start=search(start);
					break;
			case 6: start=restock(start);
					printf("\n\t\t\t\t\t*******Inventory*******\t\t\t\t\t\t\t\n");
					printf("---------------------------------------------------------------------------------------------------------------------------\n");
					printf("Name\t\tCode\t\tIn stock\tCost Price\tSales Price\tQuantity sold\tTotal cost\tTotal sale\tprofit\n");
					printf("----------------------------------------------------------------------------------------------------------------------------\n");
					start=display(start);
					break;
			case 7: start=restock(start);
					start=report(start);
				break;
